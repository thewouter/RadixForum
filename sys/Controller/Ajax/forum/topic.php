...
class topic {
    ...
    public function create() {
		...
		$opkomst = date("Y-m-d");
		if(isset($_POST['opkomst'])) {
			$opkomst=$_POST['opkomst'];
		}
        ...
        if (isset($_POST['title']) && isset($_POST['cat']) && isset($_POST['imesg']) && isset($_POST['omesg'])) {
            ...
            if ($sticky == 'yes' && $user->can('make sticky')) {
                ...
                if ($frontpage == 'yes') {
                    ...
                    $tid = $topic->ins_topic($catid, $title, $needsModeration, \CODOF\Forum\Forum::STICKY, $opkomst);
                } else {
                    $tid = $topic->ins_topic($catid, $title, $needsModeration, \CODOF\Forum\Forum::STICKY_ONLY_CATEGORY, $opkomst);
                }
            } else {

                $tid = $topic->ins_topic($catid, $title, $needsModeration, \CODOF\Forum\Forum::APPROVED, $opkomst);
            }
